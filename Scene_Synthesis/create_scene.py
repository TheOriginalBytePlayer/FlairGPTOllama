...
def call_ollama(prompt, model="llama3.2", stream=False):
    """Helper function to call Ollama's API and return the response content."""
...
lang1output = call_ollama(lang_prompt1)
...
lang2output = call_ollama(lang_prompt2)
...
lang3output = call_ollama(lang_prompt3)
...
lang4output = call_ollama(lang_prompt4)
...
lang5output = call_ollama(lang_prompt5)
...
lang6output = call_ollama(lang_prompt6)
...
against a wall, or which side should be accessible for use (Most objects will have an accessible constraint -
 ...
 Only give these constraints and considerations, no other information. """
lang7output = call_ollama(lang_prompt7)
...
lang8output = call_ollama(lang_prompt8)
...
lang9output = call_ollama(lang_prompt9)
...
cleaning1output = call_ollama(cleaning_prompt1)
...
cleaning2output = call_ollama(cleaning_prompt2)
...
cleaning3output = call_ollama(cleaning_prompt3)
...
cleaning4output = call_ollama(cleaning_prompt4)   
...
def get_primary_function_calls():
    ...
    if response5.status_code == 200:
        ...
        if "```" in response_data5:
            list_objects = (response_data5).split("\n")[1:-1]
            response5_1 = ""
            for i in range(len(list_objects)):
                response5_1 += list_objects[i] + "\n"
        else: 
            
            response5_1 = response_data5
...
def get_secondary_function_calls():
    ...
    if response6.status_code == 200:
        ...
        if "```" in response6.json():
            list_objects = (response6.json()).split("\n")[1:-1]
            response6_1 = ""
            for i in range(len(list_objects)):
                response6_1 += list_objects[i] + "\n"
        else: 
            
            response6_1 = response6.json()
...
def get_tertiary_function_calls():
    ...
    if response8.status_code == 200:
        ...
        if "```" in response8.json():
            list_objects = (response8.json()).split("\n")[1:-1]
            response8_1 = ""
            for i in range(len(list_objects)):
                response8_1 += list_objects[i] + "\n"
        else: 
            
            response8_1 = response8.json()
...
STYLEoutput = call_ollama(lang_prompt10)
